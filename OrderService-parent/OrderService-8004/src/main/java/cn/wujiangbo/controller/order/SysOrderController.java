...
public class SysOrderController extends BaseController{
    ...
    public JSONResult save(@RequestBody ShopVo shopVo){
        /** 假装有入参校验
         *
         */
        SysOrder order = new SysOrder();
        String s = MyTools.getRandomChar(2) + MyTools.getRandomNum(8);
            order.setOrderId(s);
        BigDecimal zongJia = new BigDecimal(shopVo.getAccount()).multiply(shopVo.getPrice());
        order.setTotalPrices(zongJia);
        order.setShopName(shopVo.getShopName());
        order.setDescribee(shopVo.getDescribee());
